...
def count_required(step):
    def price_in_materials(x):
        ...
        return run_price
    # FIXME probably runs_required are not needed here
    def runs_required(x):
        run_size  =  np.minimum(x.quantity,x.run)
        jobs_required = (np.ceil(x.quantity / run_size)).astype('int32')
...
def append_products(step):
    return step.set_index('typeID').join(withdrawed_products().set_index('productTypeID'), rsuffix='_product', how='inner')

def append_materials(step):
    return step.set_index('typeID').join(alterated_materials(),how='inner',rsuffix='_materials')
...
def ultimate_decompose(product,run_size):
    ...
    init_table['quantity'] = run_size + (-run_size%my_collection.prints[product].runs)

    atomic = pd.DataFrame([], columns=['typeID', 'quantity','basePrice']).set_index('typeID')

    step = init_table
    steps = [step]

    while True:
        step = step.merge(collection, on='typeID', how = 'left').fillna(value={'me':1,'te':1,'run':2**10})
        ...
        step = step[['quantity']].reset_index().rename({'index':'typeID'}, axis='columns')

        atomic = atomic.append(step[~step['typeID'].isin(withdrawed_products()['productTypeID'])])
        ...
        if step.shape[0] == 0:
            break
    a = atomic[['typeID','quantity']].astype({'typeID':'int32'}).groupby('typeID').sum()
    a = a.join(types.set_index('typeID'),lsuffix='-atom_')[['quantity','typeName']]
    return steps, a
...
def create_production_schema(product, run_size):
    ...
    for s in steps:
        ...
        yield v[['typeID','typeName','quantity','runs_required',]]
